...
const biosDuration = computed(() => {
  const d = widgetSettings.value?.osConfig?.win95?.timings?.bios ?? 3000;
  return `${Number(d)}ms`;
});
</script>
...
<template>
  ...
  <div class="bios-content award" :style="{ '--bios-duration': biosDuration }">
    ...
    <!-- BIOS visual progress (fills over configured BIOS time) -->
...
<style scoped>
/* Focused Award BIOS (Windows 95) styles
   - Removed unrelated OS styles (AMI/Phoenix/XP/UEFI)
   - Adds a config-driven BIOS progress fill (uses --bios-duration)
*/
